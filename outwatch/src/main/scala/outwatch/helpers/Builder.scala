...
trait AttributeBuilder[-T, +A] extends Any {
  ...
  @inline final def <--(source: Seq[T]): Seq[A] = source.map(assign)
  @inline final def <--[F[_] : ObservableLike](source: F[_ <: T]): Observable[A] = ObservableLike[F].toObservable(source).map(assign(_))

  @inline final def <---[F[_] : ObservableLike, G[+_]: Functor](source: F[_ <:G[_ <: T]]): Observable[G[A]] = ??? //ObservableLike[F].toObservable(source).map(assign(_))

  @deprecated("Use builder <--- source instead", "1.0.0")
  @inline final def <--?[F[+_] : Assignable](source: F[Option[T]]): ModifierM[Any] = ??? //builder <-- source
...
object AttributeBuilder {
  ...
  // @inline implicit class AttributeBuilderOperations[T](val builder: AttributeBuilder[T, ModifierM[Any]]) extends AnyVal {
  // }
